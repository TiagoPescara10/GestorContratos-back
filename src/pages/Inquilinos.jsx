...
export default function Inquilinos() {
  ...
  const inquilinos = useMemo(() => {
    const mapa = {}
    contratos.forEach(c => {
      const nombre = c.inquilinoNombre || "Sin nombre"
      if (!mapa[nombre]) {
        mapa[nombre] = {
          nombre,
          telefono: c.inquilinoTelefono || "",
          email: c.inquilinoEmail || "",
          cantidad: 0,
          activos: 0,
        }
      }
  ...
  const filtrados = useMemo(() => {
    if (!busqueda.trim()) return inquilinos
  ...
  return (
    <Box p={{ base: 4, md: 8 }} maxW="1100px" mx="auto">
      <VStack align="stretch" spacing={6}>
        ...
        <Box>
          <Heading size="lg">Inquilinos</Heading>
          <Text color="gray.500" fontSize="sm" mt={1}>
            {inquilinos.length} inquilinos · {contratos.length} contratos
          </Text>
        </Box>
        ...
        <VStack align="stretch" spacing={2}>
          ...
          {filtrados.map(p => (
            <HStack
              ...
              onClick={() => navigate(`/inquilinos/${encodeURIComponent(p.nombre)}`)}
            >
              <HStack spacing={3}>
